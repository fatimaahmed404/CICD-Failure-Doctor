...
const VALID_RATINGS: ReadonlySet<string> = new Set(["helpful", "unhelpful"]);
...
function validateRating(rating: string): asserts rating is FeedbackRating {
  if (!VALID_RATINGS.has(rating)) {
    throw new FeedbackValidationError(
      `rating must be "helpful" or "unhelpful", got: "${rating}"`,
      "rating",
    );
  }
}
...
function rowToDomain(row: FeedbackRow): FeedbackRecord {
  return {
    id: row.id,
...
export function upsertFeedback(
  ...
  const getStmt = db.prepare<[string, string]>(
    "SELECT * FROM feedback WHERE build_record_id = ? AND client_id = ?",
  );
  const row = getStmt.get(buildRecordId, clientId) as FeedbackRow;
 ...
 * Requirements: 13.3
 */
export function getFeedbackStats(): FailureCategoryStats[] {
  ...
  const statsMap = new Map<FailureCategory, FailureCategoryStats>();
