...
import { upsertGitHubToken, getGitHubToken, deleteGitHubToken } from "../db/githubTokens.js";
...
const stateStore: OAuthStateStore = new Map();
 ...
 * Requirement: 17.3
 */
export function generateOAuthState(clientId: string): string {
 ...
 * Requirement: 17.4
 */
export function validateOAuthState(clientId: string, state: string): boolean {
...
export async function exchangeCodeForToken(
  ...
  upsertGitHubToken(clientId, encryptedToken, githubUsername);
 ...
 */
export function getDecryptedToken(clientId: string): string | null {
 ...
 * Requirement: 17.6
 */
export async function listUserRepos(clientId: string): Promise<GitHubRepo[]> {
  ...
  return repos
    .filter((r: any) => r.permissions?.push === true || r.permissions?.admin === true)
    .map((r: any) => ({
      name: r.name,
...
export async function connectRepository(
  clientId: string,
  repoFullName: string,
): Promise<ConnectRepoResult | ConnectRepoPartialResult> {
  const token = getDecryptedToken(clientId);
  if (!token) {
    throw new TokenNotFoundError("No GitHub token found for this client");
  }
  ...
  function sealSecret(secretValue: string): string {
  ...
  if (!workflowRes.ok) {
    ...
    return {
      ...
      manualSetupUrl:
        "https://github.com/your-org/cicd-failure-doctor#manual-setup",
    };
  }
 ...
 * Requirement: 17.20
 */
export function disconnectGitHub(clientId: string): void {
  deleteGitHubToken(clientId);
}

// ── Workflow file template ────────────────────────────────────────────────────

function buildWorkflowContent(): string {
 ...
 */
export function handleGitHubApiErrors(res: Response): void {
  if (res.ok) return;
  ...
  if (res.status === 401 || res.status === 403) {
    throw new InvalidTokenError(
      "GitHub token is no longer valid. Please reconnect your account.",
    );
  }
}

function handleSecretError(res: Response): void {
  if (res.ok) return;

  if (res.status === 403) {
    throw new InsufficientPermissionsError(
      "You do not have permission to create secrets in this repository",
    );
  }
  if (res.status === 404) {
