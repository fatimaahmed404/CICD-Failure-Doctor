...
async function sendSlackNotification(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[NotificationService] Slack webhook returned ${response.status}: ${await response.text()}`,
      );
      return false;
    }

    console.log(
      `[NotificationService] Slack notification sent for category: ${category}`,
    );
    return true;
  } catch (error) {
    console.error(
      `[NotificationService] Slack notification failed:`,
...
async function sendEmailNotification(
  ...
  try {
    ...
    console.error(
      `[NotificationService] Email notification failed:`,
      error instanceof Error ? error.message : String(error),
    );
    return false;
  }
