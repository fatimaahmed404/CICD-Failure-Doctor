...
export function truncateLog(
  ...
  const tailLineNumbers = new Set<number>();
  ...
  const keywordOnlyEntries: Array<{ lineNumber: number; text: string }> = [];

  for (let i = 0; i < originalLineCount; i++) {
    if (keywordOnlyEntries.length >= keywordLines) break;
    if (tailLineNumbers.has(i)) continue; // already covered by tail
