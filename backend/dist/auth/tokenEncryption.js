...
exports._resetKeyForTesting = _resetKeyForTesting;
...
function initEncryptionKey() {
    ...
    if (rawKey && rawKey.trim() !== "") {
        ...
        return (0, crypto_1.scryptSync)(rawKey, FIXED_SALT, KEY_BYTES, { N: 16384, r: 8, p: 1 });
    }
    // Req 17.17: ephemeral key — warn and continue
    console.warn("[startup] TOKEN_ENCRYPTION_KEY not set — tokens will not persist across restarts");
    return (0, crypto_1.randomBytes)(KEY_BYTES);
}
// Initialised once at module load time; exported for testing purposes.
exports.encryptionKey = initEncryptionKey();
/**
 * Reset the module-level key — intended for use in tests only.
 * @internal
 */
function _resetKeyForTesting() {
    exports.encryptionKey = initEncryptionKey();
}
// ── Public API ────────────────────────────────────────────────────────────────
/**
...
function encryptToken(plaintext) {
    ...
    const cipher = (0, crypto_1.createCipheriv)(ALGORITHM, exports.encryptionKey, iv);
...
function decryptToken(encrypted) {
    ...
    if (parts.length !== 3) {
        throw new Error(`Invalid encrypted token format: expected '<iv>:<authTag>:<ciphertext>', got ${parts.length} segment(s)`);
    }
    ...
    const decipher = (0, crypto_1.createDecipheriv)(ALGORITHM, exports.encryptionKey, iv);
