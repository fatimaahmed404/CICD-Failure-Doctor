...
function timeoutReject(ms) {
    return new Promise((_, reject) => setTimeout(() => reject(new SLATimeoutError(`Diagnosis SLA timeout: exceeded 120 s`)), ms));
}
        ...
        try {
            ...
            if (err instanceof llmClient_js_1.NetworkError) {
                ...
                if (attempt < MAX_ATTEMPTS) {
                    console.warn(`[jobQueue] NetworkError for ${buildRecordId} (attempt ${attempt}/${MAX_ATTEMPTS}), retrying in ${RETRY_DELAY_MS}ms:`, err.message);
 ...
 * Requirements: 3.5
 */
async function processJobWithSLA(job) {
    try {
